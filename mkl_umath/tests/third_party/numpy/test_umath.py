# This file includes tests from numpy:
# https://github.com/numpy/numpy/blob/maintenance/2.3.x/numpy/_core/tests/test_umath.py
# while mkl_umath.use_in_numpy() is activated to use mkl_umath in Numpy
# any changes compared to the original file are tagged with CHANGED comment.
...
import mkl_umath  # CHANGED: For enabling mkl_umath in Numpy
...
@pytest.mark.parametrize("arr", [
    np.arange(2),
    np.array([0, 1]),  # CHANGED: np.matrix -> np.array, To avoid warning
    np.array([[0, 1], [2, 5]]),  # CHANGED: np.matrix -> np.array, To avoid warning
    ])
...
@pytest.mark.skip("should be added when numpy-2.4 is released")  # CHANGED
